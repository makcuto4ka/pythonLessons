    ...
    try:
        ...
        for i in navi:
            if i == 'L':
                ...
                print('Ошибка! Неверно введён путь к листочку.')
                return copy_tree[0]
...
def show_tree(tree):
    ...
    for i in range(len(s_tree)):
        ...
        for j in range(len(s_tree[i])):
            if s_tree[i][j] != None:
                draw.text((w*(kj+1)//(len(s_tree[i])+1)-len(s_tree[i][j])
                          * size//4, ki*50), s_tree[i][j], 'black', font=font)
            if i+1 != len(s_tree):
                if s_tree[i][j] != None and s_tree[i+1][j] != None:
                    img1 = ImageDraw.Draw(img)
                    img1.line([(w*(kj+1)//(len(s_tree[i])+1), ki*50+20),
                              (w*(kj2+1)//(2**(ki+1)+1), ki*50+50)], fill='black', width=4)
                if s_tree[i][j] != None and s_tree[i+1][j+1] != None:
                    img1 = ImageDraw.Draw(img)
                    img1.line([(w*(kj+1)//(len(s_tree[i])+1), ki*50+20),
                              (w*(kj2+2)//(2**(ki+1)+1), ki*50+50)], fill='black', width=4)
            kj += 1
            kj2 += 2
        ki += 1
    img.show()
